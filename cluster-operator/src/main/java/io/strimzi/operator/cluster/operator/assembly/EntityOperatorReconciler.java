    ...
    protected Future<Void> entityOperatorRole() {
        return roleOperator
                .reconcile(
                        ...
                        reconciliation.namespace(),
                        KafkaResources.entityOperatorDeploymentName(reconciliation.name()),
                        shouldInstallEntityOperator() ? entityOperator.generateRole(reconciliation.namespace(), reconciliation.namespace()) : null
                ).mapEmpty();
    }
    ...
    protected Future<Void> topicOperatorRole() {
        if (entityOperator != null && entityOperator.topicOperator() != null) {
            ...
            if (!topicOperatorHasValidConfig()) {
                ...
                // Creation case: generate Role for watched namespace
                role = entityOperator.generateRole(reconciliation.namespace(), namespace);
            } else {
